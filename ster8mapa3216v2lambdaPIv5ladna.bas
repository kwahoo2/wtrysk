...
Config Portd.4 = Output                                     'wyjscie kroku dla ster. silnika krok.
Config Portd.3 = Output                                     'wyscie kierunku dla ster. silnika krok.

Wyjsciezaplon Alias Portd.7                                 'alias wyjscia syganlu do EDIS
Wyjsciewtrysk1 Alias Portd.6                                'wtryskiwacz 1
Wyjsciewtrysk2 Alias Portd.5                                'wtryskiwacz 2

Config Portb.1 = Output

Pompa Alias Portb.1                                         'przekaŸnik pompy paliwa
...
Dim Wzap(256) As Byte                                       'tablica kata zaplonu
...
Dawkawolne = 45                                             'wartosc poczatkowa dawki biegu jalowego

Config Timer1 = Timer , Prescale = 256                      'licznik okresu pomiedzy dwoma impusami z EDIS
Config Timer0 = Timer , Prescale = 64                       'licznik - zaplon 64=>1cykl= 8us   max=2ms
Config Timer2 = Timer , Prescale = 256                      'licznik - wtrysk 256=>1cykl= 32us  max=8ms
Config Pind.2 = Input                                       'wejscie impulsu z EDIS
Config Int0 = Falling                                       'reakcja na spadek
Enable Interrupts
Enable Int0
Enable Timer0
Enable Timer2

Config Adc = Single , Prescaler = 8 , Reference = Avcc      'napiecie referencyjne dla przetwornika ADC: zasilania

On Int0 Impuls                                              'konfiguracja przerwan
...
Do                                                          'glowna petla
   ...
   Lambda = Getadc(1)                                       'odczyt sygnalu sondy lambda

   Poznap(1) = Poznap(2)                                    'pomiar i usrednianie nap. zasilania
   Poznap(2) = Poznap(3)
   Poznap(3) = Poznap(4)
   Poznap(4) = Poznap(5)
   Poznap(5) = Getadc(2)
   ...
   Ntcnap = Getadc(4)                                       'odczyt sygnalu z czujnika temp. oleju

   Przepustnap = Getadc(5)                                  'odczyt z czujnika pol. przepustnicy
...
If Odliczaj = 1 Then
   ...
   Else
      Przeplyw = 0
   End If
   If Przeplyw > 347 Then                                   'nieprzekraczenie 3V (3/5*1023)-205=410
      Przeplyw = 347
   End If


   Pozycjav = Herces \ 132
   Pozycjah = Przeplyw \ 22                                 'pozycja w mapie zaplonu
   Pozycja = 16 * Pozycjav
   Pozycja = Pozycja + Pozycjah

   Pozycjavw = Herces \ 66
   Pozycjaw = 16 * Pozycjavw                                'pozycja w mapie wtrysku
   Pozycjaw = Pozycjaw + Pozycjah


   Dawka4paliwa = Lookup(pozycjaw , Dawka4)                 'odczyt z map
   ...
   End Select
      ...
      Incr Obrstart                                         'ilosc suwow od uruchomienia
      If Obrstart > 250 Then
         Obrstart = 251
      End If
      Pozntc = Ntcnap \ 64
      Ntckor = Lookup(pozntc , Ntckorek)                    'korekcja od temperatury

      Select Case Obrstart
         Case 0 To 50 : Dawka4paliwa = 200                  'bogata stala paliwa przez pierwsze 25 obrotow
         Case 51 To 150 : Dawka4paliwa = 250 - Obrstart
      Case Else :
      End Select
      Dawka4paliwa = Dawka4paliwa + Ntckor

   If Ntcnap < 400 Then                                     'dla temperatury wiekszej ni¿ okreslona
      If Przepustnap < 850 And Przepustnap > 590 Then       'tylko jesli przepustnica nie jest calkiem otwarta lub zamknieta

         If Lambda < 40 Then                                'korekcja lambda dla ubogiej
            If Bylabogata = 1 Then
               Propskok = Dawka4paliwa
               Shift Propskok , Right , 4
               Lambdakorek = Lambdakorek - Propskok         'jesli przeskok z bogatej na uboga - dodana wartosc prop. do dawki
               Reset Bylabogata
            Else
               Lambdakorek = Lambdakorek - 1
            End If
         End If
         If Lambda > 160 Then                               'korekta dla bogatej
            ...
            End If
         End If
         If Lambdakorek > 49 Then                           'zabezpieczenie przed ucieczka kor. lambda
            Lambdakorek = 49
         End If
   ...
   If Przepustnap > 850 Then
      Dawka4paliwa = Dawka4paliwa + 20                      'wzbogacanie dla pelnego otwarcia przepustnicy
   End If

   Herceswolne = Herce(11) + Herce(12)
   If Przepustnap < 580 Then                                'sterowanie dawka biegu ja³owego
   ...
   End If
      ...
      If Herces < 2004 Then                                 'ogranicznik obrotów (5000 obr/min)
         If Dawka4paliwa > 50 Then                          'wtrysk dwa razy na obrot
            ...
            Start Timer2
         Else
            Dawka4paliwa = Dawka4paliwa * 2                 'wtrysk raz na obrot
            If Pierwszy = 1 Then
               Reset Pierwszy
               Dawka4paliwa = Dawka4paliwa + Kornap
               Counter2 = 255 - Dawka4paliwa
...
If Przepustnap < 580 Then
   If Przejscie > 99 Then                                   'wysyla sygnal na sterownik silnika krokowego
...
Ntckorek:                                                   'mapa korekcji od temp.
Data 1 , 1 , 1 , 1 , 1 , 1 , 1 , 1 , 1 , 15 , 30 , 35 , 40 , 45 , 50 , 50


Impuls:                                                     'przerwanie od sygnalu z EDIS (int0)
    Herce(1) = Herce(2)
    Herce(2) = Herce(3)
    Herce(3) = Herce(4)
    Herce(4) = Herce(5)
    ...
    Start Timer1
Return
Odliczanie:                                                 'przerwanie od licznika sygnalu do EDIS (timer0)
   Stop Timer0
   Wyjsciezaplon = 0

Return

Odliczaniew:                                                'przerwanie od licznika impulsu wtrysku
   Stop Timer2
   Wyjsciewtrysk1 = 0
   Wyjsciewtrysk2 = 0
Return